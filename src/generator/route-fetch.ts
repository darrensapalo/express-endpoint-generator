...
export function GenerateGetRoute(endpoint: EndPointDefinition) {
    ...
    if (endpoint.fetch.isDisabled === true)
        return of('');

    const fileDestination = path.join(process.env.OUTPUT_DIR, endpoint.serverSubFolder, `${endpoint.fetch.filename}.ts`);

    const routeOutput = `${endpoint.modelName}Routes.get("/:identifier", ${endpoint.fetch.functionName});`;

    const now = moment();

    const writeGetRoute = readFile('templates/fetch.template.txt')
        .pipe(
